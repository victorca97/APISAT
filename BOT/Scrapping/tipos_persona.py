...
def natural_sin_representante(referencia, comprador_info: dict, data, page: Page, browser, inmatriculaciones, compradores_array):
    try:
        ...
        fechasAdquisicion_factura_cancelacion = adquisicion_info.get(
            'fechasAdquisicion_factura_cancelacion')
        ...
        datos_transferente: dict = adquisicion_info.get(
            'datosDelTransferente', {})
        ...
        if not tipo_persona:
            Registrador.error(
                f"No se encontro el dni de la persona de la inmatriculacion° {inmatriculaciones}")
            raise ValueError(
                f"El numero del documento esta vacio o no existe de la inmatriculacion°{inmatriculaciones}")

        if not isinstance(pesoBruto, int):
            raise ValueError(
                f"El campo peso bruto '{pesoBruto}' debe ser un número (entero). Se recibió: {repr(pesoBruto)}")

        if not isinstance(cilindraje, int):
            raise ValueError(
                f"El campo  cilindraje'{cilindraje}' debe ser un número (entero). Se recibió: {repr(cilindraje)}")

        if len(direccion) < 7:
            Registrador.error(
                f"Su direccion esta mal digitada ya que tiene menos de 7 caracteres la inmatriculacion {inmatriculaciones} y referencia {referencia}")
            raise ValueError(
                f"La dirección de la Empresa '{direccion}' es muy corta debe tener al menos 7 caracteres. El cliente es {nombre} con el DNI {num_documento}")
        ...
        try:
            ...
            if sat_paterno != apellido_paterno or sat_materno != apellido_materno or sat_nombre != nombre:
                ...
                if not sat_paterno and not sat_nombre:
                    Registrador.warning(
                        "El SAT no devolvió nombres. Se forzará el uso de la información de la API.")
                    usar_nombres_api = True

                # CASO B: El SAT trajo un nombre distinto a la API (Rafael vs Raphael)
                else:
                    Registrador.info(
                        "Discrepancia detectada. Realizando segunda búsqueda para confirmar nombre oficial (RENIEC)...")

                    # Manejo de alerta por si el SAT lanza un popup en el segundo intento
                    page.once("dialog", lambda dialog: dialog.accept())
                    page.locator("input[name='cmdBuscaDocuAdmi']").click()
                    # Damos 3 segundos para confirmar
                    page.wait_for_timeout(3000)

                    # ==========================================================
                    #  INYECCIÓN DE SEGURIDAD: ÚLTIMA LECTURA
                    # ==========================================================
                    sat_paterno2 = page.locator(
                        "#txtApePateAdmi").input_value().strip()
                    sat_materno2 = page.locator(
                        "#txtApeMateAdmi").input_value().strip()
                    sat_nombre2 = page.locator(
                        "#txtNombAdmi").input_value().strip()

                    if sat_paterno2 != apellido_paterno or sat_materno2 != apellido_materno or sat_nombre2 != nombre:
                        Registrador.warning(
                            "El SAT insiste con nombres distintos. ¡Forzando datos de la API!")
                        usar_nombres_api = True
                    else:
                        Registrador.info(
                            "La segunda búsqueda corrigió los nombres. Todo en orden.")
            else:
            ...
            if usar_nombres_api:
                ...
                page.locator("input[name='txtApePateAdmi']").fill(
                    apellido_paterno)
                ...
                sat_materno_final = page.locator(
                    "#txtApeMateAdmi").input_value().strip()
            ...
            if len(nombre_a_validar) > 30:
                Registrador.warning(
                    f"El nombre a ingresar excede los 30 caracteres (Longitud actual: {len(nombre_a_validar)}). Se intentará enviar de todas formas.")
            ...
            page.locator("input[name='txtCorreoElectronico']").fill(
                correoElectronico)
            ...
            if page.is_enabled("#ddlDistrito"):
                ...
                Registrador.warning(
                    "El combo de distrito no está habilitado en el SAT.")
            ...
            if fecha_nacimiento:
                try:
                    ...
                    fecha_formateada = datetime.strptime(
                        fecha_nacimiento, "%Y-%m-%d").strftime("%d/%m/%Y")
                    print(
                        f"Fecha de nacimiento a ingresar: {fecha_formateada}")
                    page.locator("input[name='txtFecNacPersona']").fill(
                        fecha_formateada)
                except ValueError as e:
                    Registrador.error(
                        f"Error al procesar la fecha de nacimiento ({fecha_nacimiento}): {e}")
            else:
                Registrador.warning(
                    "No llegó fecha de nacimiento en la API. Campo omitido.")
            ...
            input(
                "Corrige la dirección u otro dato si es necesario y presiona Enter para continuar...")
            ...
            Registrador.error(
                f"Error crítico en el bloque de llenado de persona natural: {e}")
        ...
        try:
            ...
            page.locator("input[name='txtPesoBruto']").fill(
                str(int(pesoBruto)))
            page.keyboard.press("Enter")
            ...
            print(
                f"ℹ️ Clase detectada antes de buscar modelo: {clase_vehiculo}")

            # Seleccionar modelo ULTIMO PASO
            v_modelos = f"{modelos}".strip()
            time.sleep(2)
            page.locator("input[name='txtDesModelo']").press_sequentially(
                v_modelos, delay=30)
            v_modeloCompleto = combinar_modelo_version(modelos, version)

            time.sleep(2)
            resultado_seleccion = encontrar_modelo(
                page, modelos, version, formulaRodante=formulaRodante, peso_bruto=pesoBruto)

            # DATOS DE LA ADQUISICION------------------
            page.select_option("#ddlTipoTransferencia",
                               value={tipodeadquisicion})

            fechaAdquisicion = datetime.strptime(
                fechasAdquisicion_factura_cancelacion, "%Y-%m-%d")
            ...
            if longitud_comprador_infoes > 1:
                ...
                page.locator("input[name='txtPorcentaje']").fill(
                    str(porcentaje))
            else:
            ...
            page.locator("input[name='txtValorTrasferencia']").fill(
                valorMonetario)
        ...
        encontrar_modelo2(page, modelos, version, seleccion_previa=resultado_seleccion,
                          formulaRodante=formulaRodante, peso_bruto=pesoBruto)

        page.locator("input[name='txtFechaAdquiV']").fill(
            str(fecha_formateada1))
        page.locator("input[name='txtValorTrasferenciaV']").fill(
            valorMonetario)
    ...
    # 3. EL ATRAPADOR MAESTRO: Un solo except que envía el correo sí o sí.
        ...
        destinos = ["practicantes.sistemas@notariapaino.pe", "jmallqui@notariapaino.pe",
                    "jmallqui@autohub.pe", "administracion@autohub.pe"]
...
def juridica_con_representante(referencia, comprador_info: dict, data, page: Page, browser, inmatriculaciones, compradores_array):
    try:
        ...
        correoElectronico = comprador_info.get('correoElectronico', None)
        correoElectronicoAlternativoR = comprador_info.get(
            'correoElectronicoAlternativo', None)
        fecha_nacimiento = comprador_info.get('fechaDeNacimiento', "")
        domicilio_fiscal: dict = comprador_info.get('domicilioFiscal', {})
        distrito = domicilio_fiscal.get('distrito')
        direccion = domicilio_fiscal.get('direccion')
        ...
        domicilio_fiscal: dict = representante_info.get(
            'domicilioDeRelacionado', {})
        ...
        if len(direccion) < 7:
            Registrador.error(
                f"Su direccion esta mal digitada ya que tiene menos de 7 caracteres la inmatriculacion {inmatriculaciones} y referencia {referencia}")
            raise ValueError(
                f"La dirección de la Empresa '{direccion}' es muy corta debe tener al menos 10 caracteres. El cliente es {nombre} con el DNI {num_documento}")

        if len(direccionR) < 7:
            Registrador.error(
                f"Su direccion esta mal digitada ya que tiene menos de 7 caracteres la inmatriculacion {inmatriculaciones} y referencia {referencia}")
            raise ValueError(
                f"La dirección del representate '{direccion}' es muy corta debe tener al menos 10 caracteres. El cliente es {nombreR} con el DNI {num_documentoR}")
        ...
        try:
            ...
            page.locator("input[name='txtTelefono1']").fill(
                telefonoFijo if telefonoFijo else "")
            page.locator("input[name='txtTelefono2']").fill(
                celular if celular else "")
            page.locator("input[name='txtCorreoElectronico']").fill(
                correoElectronico if correoElectronico else "")
            ...
            if page.locator("#ddlDistrito").is_enabled():
                ...
                Registrador.warning(
                    "El combo de distrito de la empresa no está habilitado.")

            input(
                "Corrige los datos de la EMPRESA si es necesario y presiona Enter para continuar...")
            ...
            if sat_pate != data_rep["paterno"] or sat_mate != data_rep["materno"] or sat_nomb != data_rep["nombre"]:
                ...
                if not sat_pate and not sat_nomb:
                    Registrador.warning(
                        "SAT vacío para representante. Se forzará el uso de la API.")
                    usar_api_rep = True
                # CASO B: Discrepancia. Segunda búsqueda de confirmación.
                else:
                    Registrador.info(
                        "Discrepancia en representante. Realizando segunda búsqueda para confirmar...")
                    page.once("dialog", lambda d: d.accept())
                    page.locator("#cmdBuscaDocuRel").click()
                    page.wait_for_timeout(3000)

                    # Volvemos a leer lo que trajo el SAT tras el segundo intento
                    sat_pate2 = page.locator(
                        "#txtApePateRela").input_value().strip()
                    sat_mate2 = page.locator(
                        "#txtApeMateRela").input_value().strip()
                    sat_nomb2 = page.locator(
                        "#txtNombRela").input_value().strip()
                    ...
                    if sat_pate2 != data_rep["paterno"] or sat_mate2 != data_rep["materno"] or sat_nomb2 != data_rep["nombre"]:
                        Registrador.warning(
                            "El SAT insiste con otro representante. ¡Sobreescribiendo con la API a la fuerza!")
                        usar_api_rep = True
                    else:
                        Registrador.info(
                            "La segunda búsqueda corrigió los nombres. Todo en orden.")
            ...
            if usar_api_rep:
                page.locator("input[name='txtApePateRela']").fill(
                    data_rep["paterno"])
                ...
                page.locator("input[name='txtNombRela']").fill(
                    data_rep["nombre"])
            else:
                if not page.locator("#txtApeMateRela").input_value().strip():
                    page.locator("input[name='chkSinApeMatRela']").check()
                nombre_final_rep = page.locator(
                    "#txtNombRela").input_value().strip()

            if len(nombre_final_rep) > 30:
                Registrador.warning(
                    f"El nombre del representante excede 30 caracteres (Longitud: {len(nombre_final_rep)}).")

            # 5. LLENADO DEL RESTO DE DATOS (Manda la API)
            page.locator("input[name='txtTelefono1Rela']").fill("")
            page.locator("input[name='txtTelefono2Rela']").fill(
                data_rep["celular"])
            page.locator("input[name='txtCorreoElectronicoRela']").fill(
                data_rep["correo"])

            if page.is_enabled("#ddlDistritoRela"):
                page.select_option("#ddlDistritoRela",
                                   value=data_rep["distrito"])
                page.locator("input[name='txtDireccionRela']").fill(
                    data_rep["direccion"])
            else:
                Registrador.warning(
                    "El combo de distrito del representante no está habilitado.")

            if data_rep["fecha"]:
                try:
                    f_nac_rep = datetime.strptime(
                        data_rep["fecha"], "%Y-%m-%d").strftime("%d/%m/%Y")
                    ...
                    Registrador.error(
                        f"Error al procesar la fecha de nacimiento del rep: {e}")

            # 6. FINALIZACIÓN
            input(
                "Corrige algún dato del representante si es necesario y presiona Enter para continuar...")
            Registrador.info(
                "Terminé la hoja completa de la Persona Jurídica.")
            ...
            Registrador.error(
                f"Error crítico procesando a la Persona Jurídica: {e}")
            try:
                page.locator("#lnkRegresar").click()
            except:
                pass
        ...
        try:
            ...
            fecha_formateada1 = fecha_formateada1.replace("-", "/")
            print(fecha_formateada1)
            # input("Ingresar fecha de adquisición...")
            page.locator("input[name='txtFechaAdqui']").fill(fecha_formateada1)
        ...
        # input("Validar modelo y version")
        Registrador.info("Termine la parte final de la hoja")
        time.sleep(2)
...
def sociedadconyugal(referencia, comprador_info, data, page: Page, browser, inmatriculaciones, compradores_array: list[dict]):
    ...
    try:
        ...
        if len(compradores_array) >= 1:
            ...
            datos_transferente: dict = adquisicion_info.get(
                'datosDelTransferente', {})
            distritoUbicacion = datos_transferente.get(
                'distritoUbicacion', None)
            ...
            domicilio_fiscal2: dict = comprador2_info.get(
                'domicilioFiscal', {})
            ...
            if len(direccion) < 7:
                Registrador.error(
                    f"Su direccion esta mal digitada ya que tiene menos de 7 caracteres la inmatriculacion {inmatriculaciones} y referencia {referencia}")
                raise ValueError(
                    f"La dirección del Conyuje '{direccion}' es muy corta debe tener al menos 10 caracteres. El cliente es {nombre} con el DNI {num_documento}")

            if len(direccion2) < 7:
                Registrador.error(
                    f"Su direccion esta mal digitada ya que tiene menos de 7 caracteres la inmatriculacion {inmatriculaciones} y referencia {referencia}")
                raise ValueError(
                    f"La dirección del Conyuje '{direccion2}' es muy corta debe tener al menos 10 caracteres. El cliente es {nombre2} con el DNI {num_documento2}")
            ...
            try:
                ...
                def leer_alerta_sat(dialog):
                    ...
                    if "como cónyuge" in texto_alerta or "se colocará el número de documento ingresado" in texto_alerta:
                        ...
                        Registrador.warning(
                            "El SAT decidió INVERTIR los roles.")
                    elif "sociedad conyugal" in texto_alerta:
                        estado_sat = "PRECARGADO"
                        Registrador.warning(
                            "El SAT precargó la sociedad conyugal (Roles Normales).")
                ...
                # ==============================================================================
                sat_pate_admi = page.locator(
                    "#txtApePateAdmi").input_value().strip()
                sat_mate_admi = page.locator(
                    "#txtApeMateAdmi").input_value().strip()
                sat_nomb_admi = page.locator(
                    "#txtNombAdmi").input_value().strip()
                ...
                if sat_pate_admi != admin["paterno"] or sat_mate_admi != admin["materno"] or sat_nomb_admi != admin["nombre"]:
                    ...
                    if sat_pate_admi != admin["paterno"] or sat_mate_admi != admin["materno"] or sat_nomb_admi != admin["nombre"]:
                        Registrador.warning(
                            "¡Forzando nombres de la API en Fase 1!")
                        usar_api_admi = True
                ...
                if usar_api_admi:
                    ...
                    if not admin["materno"]:
                        ...
                        page.locator("input[name='txtApeMateAdmi']").fill(
                            admin["materno"])
                    page.locator("input[name='txtNombAdmi']").fill(
                        admin["nombre"])
                else:
                ...
                if page.is_enabled("#ddlDistrito"):
                    ...
                    page.locator("input[name='txtDireccion']").fill(
                        admin["direccion"])

                if admin["fecha"]:
                    try:
                        f_nac_arriba = datetime.strptime(
                            admin["fecha"], "%Y-%m-%d").strftime("%d/%m/%Y")
                        page.locator("input[name='txtFecNacPersona']").fill(
                            f_nac_arriba)
                    except ValueError as e:
                        Registrador.error(
                ...
                if estado_sat == "NORMAL":
                    page.locator("#btnNuevaBusquedaRel").click()
                    page.select_option("#ddlTipoDocuRela",
                                       value=rela["tipo_doc"])
                    page.locator("input[name='txtDocuRela']").fill(
                        rela["num_doc"])
                ...
                sat_pate_r1 = page.locator(
                    "#txtApePateRela").input_value().strip()
                sat_mate_r1 = page.locator(
                    "#txtApeMateRela").input_value().strip()
                sat_nomb_r1 = page.locator(
                    "#txtNombRela").input_value().strip()
                ...
                if sat_pate_r1 != rela["paterno"] or sat_mate_r1 != rela["materno"] or sat_nomb_r1 != rela["nombre"]:
                    ...
                    if sat_pate_r1 != rela["paterno"] or sat_mate_r1 != rela["materno"] or sat_nomb_r1 != rela["nombre"]:
                        Registrador.warning(
                            f"Discrepancia detectada en Cónyuge | API: {rela['nombre']} {rela['paterno']} vs SAT: {sat_nomb_r1} {sat_pate_r1}. ¡Forzando API!")
                        usar_api_r1 = True
                ...
                if rela["fecha"]:
                    try:
                        f_nac_abajo = datetime.strptime(
                            rela["fecha"], "%Y-%m-%d").strftime("%d/%m/%Y")
                        selector_fecha_abajo = "input[name='txtFecNacRelacionado']"

                        if page.locator(selector_fecha_abajo).is_visible():
                            page.locator(selector_fecha_abajo).fill(
                                f_nac_abajo)

                    except ValueError as e:
                        Registrador.error(
                            f"Error al procesar la fecha de nacimiento ({fecha_nacimiento2}): {e}")
                else:
                    Registrador.warning(
                        "No llegó fecha de nacimiento de cónyuge en la API. Campo omitido.")

                # ==============================================================================
                # FINALIZACIÓN
                # ==============================================================================
                input(
                    "Revisa los datos del Comprador y Cónyuge y presiona Enter para continuar...")
            ...
            try:
                ...
                # Formatear el objeto datetime a la cadena deseada (día-mes-año)
                fecha_formateada = datetime.strptime(
                    fechaInscripcion, "%Y-%m-%d")
                fecha_formateada = fecha_formateada.strftime("%d-%m-%Y")
                fecha_formateada = fecha_formateada.replace("-", "/")
                print(fecha_formateada)
                page.locator("input[name='txtInscripcion']").fill(
                    fecha_formateada)
                ...
                value_combustible = encontrar_combustible(combustible)
                page.select_option(
                    "#ddlTipoMotor", value=str(value_combustible))
                ...
                fecha_formateada1 = fecha_formateada1.replace("-", "/")
                print(fecha_formateada1)
                page.locator("input[name='txtFechaAdqui']").fill(
                    fecha_formateada1)
    ...
    try:
        if inicio_comprador:
            ...
            if sat_paterno != apellido_paterno or sat_materno != apellido_materno or sat_nombre != nombre:
                ...
                if not sat_paterno and not sat_nombre:
                    Registrador.warning(
                        "SAT vacío para Co-comprador. Forzando uso de la API.")
                    usar_nombres_api = True

                # CASO B: Discrepancia con la API
                else:
                    Registrador.info(
                        "Discrepancia detectada en Co-comprador. Forzando 2da búsqueda en RENIEC...")
                    ...
                    if sat_paterno2 != apellido_paterno or sat_materno2 != apellido_materno or sat_nombre2 != nombre:
                        Registrador.warning(
                            "SAT terco con Co-comprador. ¡Activando permiso para usar la API!")
                        usar_nombres_api = True  # <--- ¡AQUÍ LE DAMOS PERMISO AL PASO 3!
                    else:
                        Registrador.info(
                            "Nombres oficiales confirmados por el SAT.")
            else:
            ...
            if usar_nombres_api:
                ...
                nombre_a_validar = page.locator(
                    "#txtNombAdmi").input_value().strip()

            # Límite de 30 caracteres
            if len(nombre_a_validar) > 30:
                Registrador.warning(
                    f"Nombre de Co-comprador excede 30 chars (Longitud: {len(nombre_a_validar)}). Se intentará enviar igual.")
            ...
            if fecha_nacimiento:
                try:
                    ...
                    Registrador.error(
                        f"Error al procesar la fecha de nacimiento: {e}")
            else:
                Registrador.warning(
                    "No se proporcionó fecha de nacimiento. Campo omitido.")
            ...
            if sat_pate_admi != apellido_paterno or sat_mate_admi != apellido_materno or sat_nomb_admi != nombre:
                ...
                if not sat_pate_admi and not sat_nomb_admi:
                    Registrador.warning(
                        "SAT vacío para el Co-comprador. Forzando uso de la API.")
                    usar_api_admi = True

                # CASO B: Discrepancia con la API
                else:
                    Registrador.info(
                        "Discrepancia en Co-comprador. Forzando 2da búsqueda en RENIEC...")
                    ...
                    #  INYECCIÓN DE SEGURIDAD: ÚLTIMA LECTURA
                    # ==========================================================
                    sat_pate_admi2 = page.locator(
                        "#txtApePateAdmi").input_value().strip()
                    sat_mate_admi2 = page.locator(
                        "#txtApeMateAdmi").input_value().strip()
                    sat_nomb_admi2 = page.locator(
                        "#txtNombAdmi").input_value().strip()

                    if sat_pate_admi2 != apellido_paterno or sat_mate_admi2 != apellido_materno or sat_nomb_admi2 != nombre:
                        Registrador.warning(
                            "SAT terco con el Co-comprador. ¡Forzando datos de la API!")
                        usar_api_admi = True
                    else:
                        Registrador.info(
                            "La segunda búsqueda corrigió los nombres. Todo en orden.")
            ...
            if usar_api_admi:
                ...
                nombre_val_admi = page.locator(
                    "#txtNombAdmi").input_value().strip()

            # Límite de 30 caracteres
            if len(nombre_val_admi) > 30:
                Registrador.warning(
                    f"Nombre de Co-comprador excede 30 chars (Longitud: {len(nombre_val_admi)}).")
            ...
            if fecha_nacimiento:
                ...
                Registrador.warning(
                    "No se ingresó fecha de nacimiento. Campo omitido.")

            # 5. FINALIZACIÓN DE LA HOJA
            input(
                "Corrige los datos del co-comprador si es necesario y presiona Enter para continuar...")
            ...
            longitud_comprador_infoes = len(comprador_infoes_list)
            print(
                f"Total de compradores en la lista: {longitud_comprador_infoes}")
